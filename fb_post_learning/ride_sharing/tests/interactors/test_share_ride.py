...
from ride_sharing.interactors.ShareRideInteractor import ShareRideInteractor
...
class TestShareRide:
    ...
    def test_share_ride(self):
        ...
        share_ride_dto = ShareRideDTO(
            from_place = "Kurnool",
            to_place = "Hyderabad",
            start_datetime = datetime(year=2020, month=10, day=19, hour=12),
            number_of_seats_required = 2,
            car_no = 2545
        )

        # Act
        interactor.share_ride(share_ride=share_ride_dto)

        # Assert
        storage.share_ride.assert_called_once_with(share_ride=share_ride_dto)
'''    
